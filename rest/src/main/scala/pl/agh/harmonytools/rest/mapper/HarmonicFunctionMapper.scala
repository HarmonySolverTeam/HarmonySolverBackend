...
object HarmonicFunctionMapper extends Mapper[HarmonicFunction, HarmonicFunctionDto] {
  override def mapToModel(dto: HarmonicFunctionDto): HarmonicFunction = {
    HarmonicFunction(
      ...
      dto.position.map(ChordComponentManager.chordComponentFromString(_)),
      dto.revolution.map(ChordComponentManager.chordComponentFromString(_)),
      dto.delays.map(delays => delays.map(d => DelayMapper.mapToModel(d))).getOrElse(List.empty).toSet,
      dto.extra.map(e => e.map(ChordComponentManager.chordComponentFromString(_))).getOrElse(List.empty).toSet,
      dto.omit.map(o => o.map(ChordComponentManager.chordComponentFromString(_))).getOrElse(List.empty).toSet,
      dto.isDown.getOrElse(false),
  ...
  override def mapToDTO(model: HarmonicFunction): HarmonicFunctionDto = {
    HarmonicFunctionDto(
      ...
      Some(model.delay.toList.map(d => DelayMapper.mapToDTO(d))),
