...
trait SoftRulesAbstractTest extends FunSuite with Matchers with TestUtils {
  ...
  test("Double prime test") {
    ...
    val ch1 = Chord(Note(72, C, prime), Note(72, C, prime), Note(64, E, third), Note(60, C, prime), t1)
    val ch2 = Chord(Note(72, C, prime), Note(72, C, prime), Note(63, E, thirdDim), Note(60, C, prime), t2)

    val dummy = ch1

    val rule = DoublePrimeOrFifthRule()
    isNotBroken(rule, Connection(ch1, ch2, Option(dummy))) shouldBe true
    evaluationResult(rule, Connection(ch1, ch2, Option(dummy))) shouldBe 1997
  }
