...
trait SoftRulesAbstractTest extends FunSuite with Matchers with TestUtils {
  ...
  type C = Chord
    ...
    test("D -> T6-4 test") {
      ...
      val rule = DominantRelationConnectionRule()
      isNotBroken(rule, Connection(ch2, ch1, Option(dummy)))
    }
...
//      isBroken(rule, Connection(ch2, ch5, Option(dummy))) shouldBe true
    ...
    test("Chopin -> tonic connection test") {
      ...
      val rule = DominantRelationConnectionRule()
      isNotBroken(rule, Connection(ch3, ch1, Option(dummy))) shouldBe true
      isNotBroken(rule, Connection(ch4, ch2, Option(dummy))) shouldBe true
      isBroken(rule, Connection(ch5, ch1, Option(dummy))) shouldBe true
    }
  ...
  test("Jump in soprano line is to large") {
    ...
    isNotBroken(rule, Connection(ch2, ch1, Option(dummy))) shouldBe true
    isBroken(rule, Connection(ch3, ch2, Option(dummy))) shouldBe true
    evaluationResult(rule, Connection(ch2, ch1, Option(dummy))) shouldBe 0
    evaluationResult(rule, Connection(ch3, ch2, Option(dummy))) shouldBe 3
  }
