...
case class SpecificFunctionConnectionRule(prevBaseName: BaseFunction, currentBaseName: BaseFunction) extends SubRule[Chord] {
  override def evaluate(connection: Connection[Chord]): Double = {
    if (connection.prev.harmonicFunction.baseFunction == prevBaseName && connection.current.harmonicFunction.baseFunction == currentBaseName)
      satisfied
    else
      totallyBroken
  }
