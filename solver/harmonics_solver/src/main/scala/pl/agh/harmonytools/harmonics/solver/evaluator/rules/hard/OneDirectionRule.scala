...
import pl.agh.harmonytools.harmonics.solver.evaluator.rules.satisfied
...
case class OneDirectionRule(evaluationRatio: Double = 1.0) extends AnyRule[Chord](evaluationRatio) {
  override def evaluate(connection: Connection[Chord]): Double = {
    ...
    if (currentChord.hasCorrespondingNotesLowerThan(prevChord) || currentChord.hasCorrespondingNotesUpperThan(prevChord)) {
      evaluationRatio * 35
    } else satisfied
  }
