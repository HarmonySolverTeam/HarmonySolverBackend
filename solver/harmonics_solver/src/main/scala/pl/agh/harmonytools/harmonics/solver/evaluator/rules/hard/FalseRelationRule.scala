...
import pl.agh.harmonytools.harmonics.solver.evaluator.rules.{satisfied, voicePairs}
...
case class FalseRelationRule(evaluationRatio: Double = 1.0) extends AnyRule[Chord](evaluationRatio) {
  ...
  private def causedBySopranoOrBassSettings(prevChord: Chord, currentChord: Chord, prevVoice: Int, currentVoice: Int): Boolean = {
    prevVoice == 3 || currentVoice == 3 || (prevChord.countBaseComponents(3) == 2 && prevChord.notes(prevVoice).baseChordComponentEquals(3))
  }
