...
import pl.agh.harmonytools.harmonics.solver.evaluator.rules.{ConnectionRule, specificConnectionRuleDT}
  ...
  override def evaluateIncludingDeflections(connection: Connection[Chord]): Double = {
    ...
    if (specificConnectionRuleDT.isNotBroken(connection) && prevChord.harmonicFunction.isInSecondRelation(currentChord.harmonicFunction)) {
      ...
      if (brokenFifthMoveRule(prevChord, currentChord))
        result += 20
      if (prevChord.harmonicFunction.extra.exists(_.chordComponentString == "7") && brokenSeventhMoveRule(prevChord, currentChord))
        result += 20
      if (prevChord.harmonicFunction.extra.exists(_.chordComponentString == "5>") && brokenDownFifthMoveRule(prevChord, currentChord))
        result += 20
    }
    result
  }
