package harmonytools.algorithm.graph.shortestpath.topologicalsort

import harmonytools.algorithm.graph.ScoreGraph
import harmonytools.algorithm.graph.node.{Node, NodeContent}
import harmonytools.algorithm.graph.shortestpath.{ShortestPathAlgorithm, ShortestPathAlgorithmCompanion}
...
object TopologicalSortAlgorithm extends ShortestPathAlgorithmCompanion {
  override def apply[T <: NodeContent, S <: NodeContent](graph: ScoreGraph[T, S]): ShortestPathAlgorithm[T, S] =
    new TopologicalSortAlgorithm(graph)
}
