...
case class HarmonicsSolution(
  exercise: Exercise[HarmonicFunction],
  rating: Double,
  chords: List[Chord],
  success: Boolean = true
) extends ExerciseSolution[HarmonicFunction](exercise, rating, chords, success) {
  @tailrec
  ...
  private def divideFunChanged(measure: Measure[HarmonicFunction]): List[(Int, Int)] = {
    ...
    while (i < measure.contentCount) {
      ...
      while (
        i < measure.contentCount - 1 && measure
          .contents(i)
          .hasSameFunctionInKey(measure.contents(i + 1))
      ) {
        oneFunCounter += 1
        i += 1
      }
